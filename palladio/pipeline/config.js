...
// agent-reference.md ships inside the published package, so every path it names
// must resolve for a consumer who never clones the repo: package entry points,
// or URLs pinned to the release tag that matches this build.
const pkgVersion = JSON.parse(readFileSync(path.join(packageDir, 'package.json'), 'utf8')).version;
const repoTree = `https://github.com/bext1998/palladio-design-language-system/blob/v${pkgVersion}`;
...
function typographyCssDeclarations(record) {
  const value = tokenValue(record);
  const variablePrefix = cssVariableName(record);
  return [
    [`${variablePrefix}-font-family`, value.fontFamily.join(', ')],
    [`${variablePrefix}-font-size`, `${value.fontSize.value}${value.fontSize.unit}`],
    [`${variablePrefix}-font-weight`, String(value.fontWeight)],
    [`${variablePrefix}-line-height`, String(value.lineHeight)],
    [`${variablePrefix}-letter-spacing`, `${value.letterSpacing.value}${value.letterSpacing.unit}`]
  ];
}

function cssDeclarations(record) {
  if (tokenType(record) === 'typography') return typographyCssDeclarations(record);
  return [[cssVariableName(record), transformCssValue(record)]];
}

function cssBlock(selector, records, appliesDensityBodyFontSize = false) {
  const lines = [`${selector} {`];
  for (const record of records) {
    for (const [name, value] of cssDeclarations(record)) {
      lines.push(`  ${name}: ${value};`);
    }
  }
  if (appliesDensityBodyFontSize) {
    lines.push('  font-size: var(--pd-density-typography-body-font-size);');
  }
  lines.push('}');
  return lines.join('\n');
}
...
function renderCss({ dictionary }) {
  ...
  return [
    ...
    '',
    cssBlock(':root', [...semanticRecords, ...densityRecords('default')], true),
    '',
    cssBlock(':root[data-theme="dark"]', themeRecords),
    '',
    cssBlock(':root[data-density="compact"]', densityRecords('compact'), true),
    '',
    cssBlock(':root[data-density="spacious"]', densityRecords('spacious'), true),
    ''
  ].join('\n');
}
...
${mdTable(['Token', 'Value', '用途'], colorGroup(colorRecords, (name) => name === 'input-border'))}
...
${mdTable(['Role', 'Font', 'Size', 'Weight', 'Line-height', 'Letter-spacing', '用途'], typographyRows(textRecords))}
...
完整驗收說明、對比計算結果與已確認缺漏見 ${repoTree}/palladio/docs/accessibility/accessibility-contract.md。
