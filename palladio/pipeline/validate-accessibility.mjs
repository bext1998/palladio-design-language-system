...
function resolvedColor(name) {
  return resolveRef(darkColors[name].$value, registry);
}

const SURFACES = ['bg', 'surface', 'surface-raised', 'surface-overlay'];
const resolvedSurfaces = Object.fromEntries(SURFACES.map((name) => [name, resolvedColor(name)]));
...
console.log('  (border-strong remains a confirmed A-M2 gap — see docs/accessibility/accessibility-contract.md §11.)');
