...
- **Focus**（僅 `.pd-badge--interactive`）：`:focus-visible` 顯示已驗證的 accent outline；`--pd-color-focus-ring` 未經 `enableValidatedAccentFocusRing()` 啟用時，CSS 回退 `pd-color-border-strong`（四層表面 4.11／3.81／3.47／3.03:1）。
...
- **Disabled**：`.pd-badge--interactive:disabled` 一律把背景重設為中性 `surface-raised`，文字改為 `text-disabled`——即使套用在 `.pd-badge--accent` 上也一樣。`text-disabled` 限 inactive UI component，依 WCAG exemption 不走 A-M1 gate；此狀態另有原生 `disabled`、`cursor: not-allowed` 與無 hover／active 回饋，不只靠顏色。若不重設背景，停用的 `.pd-badge--accent.pd-badge--interactive` 會讓它疊在未經驗證的 `accent-subtle` 上，因此一律回到中性配對。
