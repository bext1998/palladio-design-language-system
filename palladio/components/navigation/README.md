...
| Focus | 鍵盤或滑鼠移入取得焦點 | `:focus-visible` 顯示內縮的已驗證 accent outline，並把背景／文字強制切回 `surface-raised`／`text-primary`。未呼叫或未通過 `enableValidatedAccentFocusRing()` 時，ring 回退 `border-strong`；它不會疊在不合格的 `surface-hover`（2.86:1）上。 |
| Disabled | 移除 `href`＋`aria-disabled="true"` | 文字改為 `pd-color-text-disabled`、`cursor: not-allowed`；因無 `href` 而自然無法被聚焦或觸發，故不只靠顏色。背景也明確還原為 `pd-color-surface`。 |
