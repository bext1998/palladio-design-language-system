...
assert.match(readme, /surface-raised/,
  'Documentation must scope the supported adjacent surfaces and call out that both variants are ' +
  'invisible on surface-raised (border-subtle and surface-raised are the same #242424).');
assert.match(readme, /水平與垂直兩種變體的線條都會完全消失/,
  'Documentation must say BOTH orientations disappear on surface-raised — a rendered check showed the ' +
  'horizontal variant is not exempt (retained spacing is not evidence of a visible line).');
assert.doesNotMatch(readme, /不受影響/,
  'Documentation must not claim any variant is unaffected on surface-raised — both are invisible there.');
