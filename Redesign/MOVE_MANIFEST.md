...
| Moved | `palladio/docs/stress-tests/content-creation/prototype/**` | `Redesign/prototypes/content-creation/prototype/**`（保留完整結構；`node_modules`／`dist` 後改為 `.gitignore`，見「明確排除」） | 未規格化、可重現的內容創作原型與測試；`NEXT_ACTION.md` 原列為未追蹤原型。 | High | Updated QA source path and moved relative imports for Palladio CSS、tokens、pipeline; `NEXT_ACTION.md` intentionally unchanged per instruction and remains an expected historical old-path reference. |
...
- `Redesign/prototypes/content-creation/prototype/dist/**`、`node_modules/**`：生成輸出與安裝依賴；曾隨整體搬移一併提交，後與 Codex 核對 Sites 部署流程（打包時才從 source build，不讀取 git 內的 dist/，且 `.openai/hosting.json` 未綁定 `project_id`）後，已改為加入 `.gitignore` 並移出版控，不作為 source。
...
- `paseo.json`：[Paseo](https://github.com/getpaseo/paseo) 本機 agent workspace 設定檔，非專案原始碼；已加入 `.gitignore` 並從版控移除。
- `SPEC_ADVERSARIAL_REVIEW-2026-09-13.md`：先前誤列為「沒有足夠證據屬於本次重構」，已更正——這份報告正是促成本次工作流調整（從規格先行改為原型／實驗先行、逐步收斂回正式規格）的直接依據，屬本次工作集。

## 驗證紀錄

1. `Resolve-Path` 驗證 repo、兩個 source 與兩個 destination 均位於 `D:\AgentCoding\PalladioDesignLanguageSystem`；destination 原先不存在。
2. 以 PowerShell `Move-Item -LiteralPath` 完成兩項搬移，未刪除檔案；`node_modules`／`dist` 隨整體原型搬移保留在磁碟上，後改為 `.gitignore` 排除於版控外。
